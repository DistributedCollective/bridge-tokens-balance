...
metrics.init({ host: 'bridge-balance-checker', prefix: 'bridge-balance-checker.' })
...
const rsk_ETHBridge = '0x1CcAd820B6d031B41C54f1F3dA11c0d48b399581'
const bsc_BSCBridge = '0xdfc7127593c8af1a17146893f10e08528f4c2aa7'
const eth_ETHBridge = '0x33C0D33a0d4312562ad622F91d12B0AC47366EE1'
...
let tokensOnXUSDAgg = [new Tokens('rsk', 'ETH', 'DAIes', '0x1A37c482465e78E6DAbE1Ec77B9a24D4236D2A11', 18, 0), new Tokens('rsk', 'ETH', 'USDCes', '0x8D1f7CbC6391D95E2774380e80A666FEbf655D6b', 18, 0), new Tokens('rsk', 'ETH', 'USDTes', '0xD9665EA8F5fF70Cf97E1b1Cd1B4Cd0317b0976e8', 18, 0), new Tokens('rsk', 'BSC', 'DAIbs', '0x6A42Ff12215a90f50866A5cE43A9c9C870116e76', 18, 0), new Tokens('rsk', 'BSC', 'USDCbs', '0x91EDceE9567cd5612c9DEDeaAE24D5e574820af1', 18, 0), new Tokens('rsk', 'BSC', 'USDTbs', '0xFf4299bCA0313C20A61dc5eD597739743BEf3f6d', 18, 0), new Tokens('rsk', 'BSC', 'BUSDbs', '0x61e9604e31a736129d7f5C58964c75935b2d80D6', 18, 0)]

let tokensOnETHsAgg = [new Tokens('rsk', 'ETH', 'ETHes', '0xFe878227c8F334038DAb20a99fC3B373fFe0a755', 18, 0), new Tokens('rsk', 'BSC', 'ETHbs', '0x30d1B36924c2c0CD1c03EC257D7FFf31bD8c3007', 18, 0)]

let tokensOnBNBsAgg = [new Tokens('rsk', 'BSC', 'BNBbs', '0xd2a826b78200c8434b957913ce4067e6e3169385', 18, 0)]
...
async function getBalances() {
	...
	for (token of myTokens) {
		...
		if (token.chain === 'rsk') {
			web3Obj = web3RSK
			if (token.bridge === 'ETH') {
				addressSC = rsk_ETHBridge
			} else {
				addressSC = rsk_BSCBridge
			}
		} else if (token.chain === 'eth') {
			web3Obj = web3ETH
			addressSC = eth_ETHBridge
		} else {
			web3Obj = web3BSC
		...
		try {
			...
			myTokens[i].amount = Math.round(amount)
...
async function getNativeBalances() {
	...
	for (token of myNativeTokens) {
		...
		try {
			let amount = await getNativeBalance(web3Obj, addressSC)
			myNativeTokens[i].amount = Math.round(amount)
...
async function getXUSDAggBalances() {
	...
	for (token of tokensOnXUSDAgg) {
		...
		try {
			let amount = await getBalance(tokenAddress, web3Obj, addressSC, decimal)
			tokensOnXUSDAgg[i].amount = Math.round(amount)
...
async function getETHSAggBalances() {
	...
	for (token of tokensOnETHsAgg) {
		...
		try {
			let amount = await getBalance(tokenAddress, web3Obj, addressSC, decimal)
			tokensOnETHsAgg[i].amount = Math.round(amount)
...
async function getBNBSAggBalances() {
	...
	for (token of tokensOnBNBsAgg) {
		...
		try {
			let amount = await getBalance(tokenAddress, web3Obj, addressSC, decimal)
			tokensOnBNBsAgg[i].amount = Math.round(amount)
...
async function printLogs() {
	...
	logger.log('info', 'Bridge-' + myNativeTokens[0].bridge + ' ' + myNativeTokens[0].symbol + ' ' + myNativeTokens[0].amount + ' ' + tokensOnETHsAgg[0].symbol + ' ' + tokensOnETHsAgg[0].amount + ' Delta: ' + `${myNativeTokens[0].amount - tokensOnETHsAgg[0].amount}`, { color: 'blue' })
	//Bridge-BSC ETH ETHbs
	logger.log('info', 'Bridge-' + myTokens[7].bridge + ' ' + myTokens[7].symbol + ' ' + myTokens[7].amount + ' ' + tokensOnETHsAgg[1].symbol + ' ' + tokensOnETHsAgg[1].amount + ' Delta: ' + `${myTokens[7].amount - tokensOnETHsAgg[1].amount}`)
	//Bridge-BSC BNB BNBbs
	logger.log('info', 'Bridge-' + myNativeTokens[1].bridge + ' ' + myNativeTokens[1].symbol + ' ' + myNativeTokens[1].amount + ' ' + tokensOnBNBsAgg[0].symbol + ' ' + tokensOnBNBsAgg[0].amount + ' Delta: ' + `${myNativeTokens[1].amount - tokensOnBNBsAgg[0].amount}`)
	//Bridge-ETH DAI DAIes
	logger.log('info', 'Bridge-' + myTokens[0].bridge + ' ' + myTokens[0].symbol + ' ' + myTokens[0].amount + ' ' + tokensOnXUSDAgg[0].symbol + ' ' + tokensOnXUSDAgg[0].amount + ' Delta: ' + `${myTokens[0].amount - tokensOnXUSDAgg[0].amount}`)
	//Bridge-ETH USDC USDCes
	logger.log('info', 'Bridge-' + myTokens[1].bridge + ' ' + myTokens[1].symbol + ' ' + myTokens[1].amount + ' ' + tokensOnXUSDAgg[1].symbol + ' ' + tokensOnXUSDAgg[1].amount + ' Delta: ' + `${myTokens[1].amount - tokensOnXUSDAgg[1].amount}`)
	//Bridge-ETH USDT USDTes
	logger.log('info', 'Bridge-' + myTokens[2].bridge + ' ' + myTokens[2].symbol + ' ' + myTokens[2].amount + ' ' + tokensOnXUSDAgg[2].symbol + ' ' + tokensOnXUSDAgg[2].amount + ' Delta: ' + `${myTokens[2].amount - tokensOnXUSDAgg[2].amount}`)
	//Bridge-BSC DAI DAIbs
	logger.log('info', 'Bridge-' + myTokens[3].bridge + ' ' + myTokens[3].symbol + ' ' + myTokens[3].amount + ' ' + tokensOnXUSDAgg[3].symbol + ' ' + tokensOnXUSDAgg[3].amount + ' Delta: ' + `${myTokens[3].amount - tokensOnXUSDAgg[3].amount}`)
	//Bridge-BSC USDC USDCbs
	logger.log('info', 'Bridge-' + myTokens[4].bridge + ' ' + myTokens[4].symbol + ' ' + myTokens[4].amount + ' ' + tokensOnXUSDAgg[4].symbol + ' ' + tokensOnXUSDAgg[4].amount + ' Delta: ' + `${myTokens[4].amount - tokensOnXUSDAgg[4].amount}`)
	//Bridge-BSC USDT USDTbs
	logger.log('info', 'Bridge-' + myTokens[5].bridge + ' ' + myTokens[5].symbol + ' ' + myTokens[5].amount + ' ' + tokensOnXUSDAgg[5].symbol + ' ' + tokensOnXUSDAgg[5].amount + ' Delta: ' + `${myTokens[5].amount - tokensOnXUSDAgg[5].amount}`)
	//Bridge-BSC BUSD BUSDbs
	logger.log('info', 'Bridge-' + myTokens[6].bridge + ' ' + myTokens[6].symbol + ' ' + myTokens[6].amount + ' ' + tokensOnXUSDAgg[6].symbol + ' ' + tokensOnXUSDAgg[6].amount + ' Delta: ' + `${myTokens[6].amount - tokensOnXUSDAgg[6].amount}`)
}
...
async function run() {
	try {
		//logger.log('info', 'info', 'Hello simple log!');
		await getBalances()
		await getNativeBalances()
		await getETHSAggBalances()
		await getBNBSAggBalances()
